...
from nnunetv2.acvl_utils.cropping_and_padding.padding import pad_nd_image
...
def predict_sliding_window_return_logits(network: nn.Module,
    ...
    with torch.no_grad():
        ...
        with torch.cuda.amp.autocast() if device.type == 'cuda' else dummy_context():
            ...
            try:
                ...
                gaussian = (gaussian.to(results_device) if use_gaussian else None)
            except RuntimeError:
