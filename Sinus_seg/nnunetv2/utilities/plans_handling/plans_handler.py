...
from nnunetv2.utilities.file_and_folder_operations import load_json, join
from nnunetv2.architectures.unet import PlainConvUNet
...
class ConfigurationManager(object):
    ...
    def preprocessor_class(self):
        ...
        if self.preprocessor_name == 'DefaultPreprocessor':
    ...
    def UNet_class(self) -> Type[nn.Module]:
        ...
        if self.UNet_class_name == "PlainConvUNet":
            unet_class = PlainConvUNet(input_channels = 1,
                      n_stages = 6,
...
class PlansManager(object):
    ...
    @property
    @lru_cache(maxsize=1)
    def label_manager_class(self):
        # from nnunetv2.utilities.label_handling.label_handling import LabelManager
        return get_labelmanager_class_from_plans(self.plans)

    def get_label_manager(self, dataset_json: dict, **kwargs):
