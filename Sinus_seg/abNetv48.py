...
class BasicBlock_Attention(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.att1 = nn.Sequential(
            nn.Conv3d(inplanes, int(planes/8), 1, 1, bias=False),
            nn.BatchNorm3d(int(planes/8)),
            nn.ReLU(inplace=True),
        )
        self.att2 = conv3x3(int(planes/8), int(planes/8), 1)
        self.att_bn1 = nn.BatchNorm3d(int(planes/8), momentum=BN_MOMENTUM)
        self.att3 = nn.Conv3d(int(planes/8), planes, 1, 1, bias=False)
        self.att_bn2 = nn.BatchNorm3d(planes, momentum=BN_MOMENTUM)
                        ...
                        if k == i - j - 1:
                            ...
                            conv3x3s.append(
                                nn.Sequential(
                                    ...
                                    nn.BatchNorm3d(num_outchannels_conv3x3, momentum=BN_MOMENTUM)
                                )
                            )
                        else:
                ...
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(
                        nn.Sequential(
                            ...
                            nn.BatchNorm3d(num_channels_cur_layer[i], momentum=BN_MOMENTUM),
                            nn.ReLU(inplace=True)
                        )
                    )
                else:
